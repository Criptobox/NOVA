...
/* NOVA v005 — Simulador del agente: habla con NOVA por el motor real.
   ...
   v010: MODO OFFLINE — sin conexión el motor local responde con la
   última copia guardada y los comandos que necesitan servidor se
   encolan en la outbox y se ejecutan solos al reconectar. */
import { useCallback, useEffect, useRef, useState } from 'react';
import { AgentButtons, Bubble, fmtClock } from './ui';
import type { OrbState } from './Brain';
import { runOfflineCommand, outboxCount } from '@/lib/nova/offline';
...
/* v010 — comandos que funcionan SIN conexión (motor local + outbox) */
const CHIPS_OFF = [
  ['ayuda', '🤖 Ayuda offline'],
  ['estado', '🧾 Estado guardado'],
  ['precio btc', '🪙 Último precio'],
  ['portafolio', '💼 Portafolio'],
  ['stock bajo', '📦 Stock bajo'],
  ['reponer 5 batería', '📥 Apuntar reposición'],
  ['venta de 2 batería a 300', '💰 Apuntar venta'],
  ['operaciones', '📊 Últimas operaciones'],
  ['recordar llamar a Ana a las 15:00', '⏰ Apuntar recordatorio'],
];
...
export function AgentView({ onState }: { onState: (s: OrbState) => void }) {
  ...
  const [localMsgs, setLocalMsgs] = useState<{ id: string; role: 'user' | 'agent'; body: string; ts: Date }[]>([]); // v010: mensajes offline
  ...
  /* v010 — seguimiento de la conexión dentro de la vista del agente */
  useEffect(() => {
    setOnline(navigator.onLine);
    setPending(outboxCount());
    const on = () => { setOnline(true); setPending(outboxCount()); };
    const off = () => setOnline(false);
    window.addEventListener('online', on);
    window.addEventListener('offline', off);
    const iv = setInterval(() => setPending(outboxCount()), 4000);
  ...
  const send = useCallback(async (payload: Record<string, unknown>) => {
    ...
    /* v010 — MODO OFFLINE: sin servidor, el motor local responde.
       Los comandos que necesitan servidor quedan encolados y se
       sincronizan solos al volver la conexión (ver page.tsx). */
    if (!navigator.onLine && t) {
      const r = runOfflineCommand(t);
      setLocalMsgs(m => [
        ...m,
        { id: `l-${Date.now()}-u`, role: 'user', body: t, ts: new Date() },
        { id: `l-${Date.now()}-a`, role: 'agent', body: r.reply, ts: new Date() },
      ]);
      setPending(outboxCount());
      onState(r.queued ? 'hablando' : 'standby');
      setBusy(false);
      setTimeout(() => onState(micOnRef.current ? 'escuchando' : 'standby'), 1500);
      return;
    }
    try {
      ...
      // v010: si el servidor no responde, cambiar a modo offline local
  ...
  return (
    <section className="view active" id="view-agente">
      <div className="viewhead">
        ...
        <p>Habla con el agente igual que un usuario de WhatsApp. Usa el mismo motor que el webhook real: cripto, alertas, recordatorios, tienda, trading, voz e imágenes. Con 🎙 activas el dictado continuo: sigue escuchando y envía cada frase como comando.</p>
      </div>
      {!online && (
        <div className="offbanner" role="status">
          📴 <span><b>Modo offline:</b> NOVA responde con la última copia guardada y <b>apunta</b> los cambios de stock y recordatorios ({pending} pendiente{pending === 1 ? '' : 's'}). Todo se sincroniza solo al reconectar.</span>
        </div>
      )}
      <article className="card">
        ...
        <div className="chatwrap">
          <div className="chatlog" ref={logRef}>
            ...
            {/* v010 — mensajes locales del modo offline (no están en el servidor) */}
            {localMsgs.map(m => (
              <div key={m.id} style={{ display: 'flex', flexDirection: 'column', alignItems: m.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '100%' }}>
                <Bubble role={m.role} body={m.body} ts={`${fmtClock(m.ts)}${m.role === 'agent' ? ' · offline' : ''}`} />
              </div>
            ))}
          ...
          <div className="chips">
            {(online ? CHIPS : CHIPS_OFF).map(([cmd, label]) => (
